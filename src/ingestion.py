...
def setup_vector_db():
    ...
    if os.path.exists(persist_file):
        print("\n[INFO] Leyendo base de datos existente...")
    ...
    if not nuevos_pdfs:
        if existing_chunks:
            print("[INFO] No hay archivos nuevos. Cargando BD desde memoria instantáneamente.")
            return DocArrayInMemorySearch.from_documents(existing_chunks, embeddings)
        else:
            print("[INFO] No hay PDFs en la carpeta 'data'.")
            return None

    # 4. Procesar ÚNICAMENTE los archivos nuevos
    print(f"[1/3] Indexando {len(nuevos_pdfs)} archivo(s) NUEVO(S)...")
    nuevos_documentos = []
    for archivo in nuevos_pdfs:
        ruta = os.path.join("data", archivo)
        print(f"  -> Leyendo nuevo: {archivo}")
        loader = PyPDFLoader(ruta)
        nuevos_documentos.extend(loader.load())

    print("[2/3] Fragmentando texto nuevo...")
    ...
    print(f"[3/3] Generando vectores actualizados... Total de fragmentos: {len(todos_los_chunks)}")
    ...
    print(f"[ÉXITO] Base de datos actualizada y persistida en {persist_file}")
    return vector_db